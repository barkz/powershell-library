...
$Workbook = $Excel.Workbooks.Open('C:\Users\Barkz\Desktop\Exchange-AD-Example.xlsx') 
$row = 2
$Worksheet = $Workbook.Worksheets.Item('Test Accounts') 

$xlCellTypeVisible = 12
$Cells = $Worksheet.UsedRange.Columns.Item(1).SpecialCells($xlCellTypeVisible) | Select-Object -Skip 1
if ($Cells) 
{
  foreach ($Cell in $Cells) 
  {
    $SamAccount = $Cell.Item($row,2).Text
    $FirstName = $Cell.Item($row,3).Text
    $LastName = $Cell.Item($row,4).Text
    $DisplayName = $FirstName + " " + $LastName
    $EmailAddress = $Cell.Item($row,2).Text+'@'+(Get-ADDomain).DNSRoot
    $Password = ConvertTo-SecureString -AsPlainText 'SomePassword' -force 

    Move-DatabasePath 'Mailbox Database 1664311763' `
        -EdbFilePath 'E:\Mailbox Database 1664311763\Mailbox Database 1664311763.edb' `
        -LogFolderPath "E:\Mailbox Database 1664311763"


    if(!(Get-ADUser -LDAPFilter "(sAMAccountName=$SamAccount)")) 
    { 
        try 
        { 
            New-Mailbox -UserPrincipalName $EmailAddress `
                -Alias $SamAccount `
                -Database "Mailbox Database 1664311763" `
                -Name $DisplayName `
                -Password $Password `
                -FirstName $FirstName `
                -LastName $LastName `
            ...
            Write-Warning 'Problem creating new mailbox.' -ErrorAction Stop
        }
    } else {
        Write-Warning 'Account Exists.' -ErrorAction Stop
    }
  }
}
